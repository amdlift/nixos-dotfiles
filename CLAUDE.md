...
- **Flatpaks are per-host.** `aspects/services/flatpak.nix` provides only the capability (nixpkgs' own `services.flatpak` declares just `enable`/`package`; `nix-flatpak` extends the same namespace with `packages`/`remotes`/`update`/`overrides`). Each host owns its own `services.flatpak.packages`, exactly like `environment.systemPackages`. `remotes` already defaults to flathub. `uninstallUnmanaged` is left `false` so hand-installed flatpaks survive — setting it `true` makes the flake the sole owner and removes every undeclared package *and remote*. Note that `packages` is a list, so definitions concatenate: an aspect could add a baseline and a host would extend it, not replace it.
