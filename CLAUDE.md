...
darwin-rebuild switch --flake .#macbook          # on the Mac; no sudo
...
modules/
  ...
  hosts/       legion5/, elitedesk/, macbook/ — machine facts + aspects to enable
...
The NixOS hosts' `hardware.nix` files are `nixos-generate-config` output wrapped in `flake.modules.nixos.<host>-hardware`, carrying real disk UUIDs; regenerating means re-wrapping, not overwriting. `macbook/hardware.nix` has no such counterpart — macOS owns the disks and boot, so it states only the platform.
...
| aaron's module defines | legion5 | elitedesk | macbook |
| --- | --- | --- | --- |
| bash aliases | ✅ | ✅ | ✅ |
...
"inert" means the options are defined and evaluate cleanly, but produce no file and no package: `elitedesk` and `macbook` both carry aaron's waybar layout and all 73 mango binds in their evaluated config, and neither generates `waybar/config` or `mango/config.conf`.
...
| macbook | `btop` | darwin's plain `btop` |
...
#    absent on elitedesk and macbook, present on legion5
...
nix eval --json .#darwinConfigurations.macbook.config.home-manager.users.aaron.xdg.configFile --apply builtins.attrNames
...
- Aspect names are kebab-case and match the file path (`aspects/core/nix-settings.nix` → `nix-settings`). Host-scoped modules are prefixed: `legion5-hardware`, `legion5-monitors`, `macbook-hardware`.
